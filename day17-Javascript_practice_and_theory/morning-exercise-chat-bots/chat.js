...
document.addEventListener('DOMContentLoaded', () => {
  const listChat = document.querySelector( '.container');
  ...
  for (const line of conversation) {
    ...
    if (side === 'local') {
      part = `<div class="${side}">
      <div class="${name}">
      <p class="local__text">${text}</p>
      </div><img class="cleverbot" src="img/cleverbot.png">
      </div>`
    } else {
      part = `<div class="${side}">
      <img class="unicorn" src ="img/unicorn.png">
      <div class="${name}">
      <p class="remote__text">${text}</p>
      </div>
      </div>`
    };
